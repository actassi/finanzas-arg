        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
          ...
          <form action={createTransaction} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-slate-300">Vencimiento (opcional)</label>
                <input
                  type="date"
                  name="due_date"
                  className="rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
                ...
                <div className="text-[11px] text-slate-400">
                  Si lo completás, ese vencimiento se usa para el “mes presupuestario” (budget_month) según tu modelo.
                </div>
              </div>
            ...
            {/* Descripción */}
            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-300">Descripción (como viene del banco)</label>
              <input
                name="description_raw"
                className="rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
                value={formState.description_raw}
                onChange={(e) => setFormState((s) => ({ ...s, description_raw: e.target.value }))}
                placeholder="MERCADOPAGO*POSTADEFUNESSA..."
                required
              />
              <p className="text-[11px] text-slate-400">
                Normalizada:{' '}
                <span className="font-mono">{normalizeDescription(formState.description_raw)}</span>
              </p>
            </div>
            ...
            <input type="hidden" name="merchant_name" value={suggestedMerchant ?? ''} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm mt-2">
              <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400 mb-1">Merchant sugerido</div>
                <div className="font-semibold">{suggestedMerchant ?? '— (sin match)'}</div>
              </div>

              <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400 mb-1">Categoría sugerida</div>
                <div className="font-mono">{getCategoryLabel(suggestedCategoryId)}</div>
              </div>
