...
export default async function TransactionsContent(props: {
  ...
  // Para filtrar por usuario en tablas, usamos getUser (id estable)
  const {
    data: { user },
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-4 space-y-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right text-sm text-slate-300">
          <div>
            Total página:{" "}
            <span className="text-slate-100 font-medium">{formatMoneyARS(pageTotals.all)}</span>
          </div>
          <div className="text-xs text-slate-400">
            Gastos: {formatMoneyARS(pageTotals.byType.expense ?? 0)} · Ingresos:{" "}
            {formatMoneyARS(pageTotals.byType.income ?? 0)} · Pagos:{" "}
            {formatMoneyARS(pageTotals.byType.payment ?? 0)}
          </div>
        </div>
      </div>
